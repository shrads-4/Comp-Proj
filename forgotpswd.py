import pygame as pg, os, homepage, time, mysql.connector, loginpage
...
def validateUser(username, email, dob, newpwd):
    ...
    if con.is_connected():
        try:
            ...
            cur.execute('select email from user_dets where username = "{}" and dob = "{}"'.format(
                username, dob))
            result = cur.fetchone()
            if result and email == result[0]:
                cur.execute(
                    'update user_dets set pwd="{}" where username="{}"'.format(newpwd, username))
                con.commit()
                return True
            else:
