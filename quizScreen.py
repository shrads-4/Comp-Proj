...
def quiz(event, screen, que="<question>", options=["option1", "option2", "option3", "option4"], length=200):
    ...
    answer1 = pygbutton.PygButton(
        (100, 260, length, 30), options[0], font=font, bgcolor=(174, 214, 220))
    answer2 = pygbutton.PygButton(
        (100, 300, length, 30), options[1], font=font, bgcolor=(174, 214, 220))
    answer3 = pygbutton.PygButton(
        (100, 340, length, 30), options[2], font=font, bgcolor=(174, 214, 220))
    answer4 = pygbutton.PygButton(
        (100, 380, length, 30), options[3], font=font, bgcolor=(174, 214, 220))
...
def queType(event, screen, T):
    ...
    screen.blit(FONT.render(
        'Choose a Topic to Answer a Question and Proceed to Next Level!', True, (0, 0, 0)), (70, 70))

    type1 = pygbutton.PygButton(
        (300, 200, 200, 30), T[0], bgcolor=(255, 154, 141))
    type2 = pygbutton.PygButton(
        (300, 300, 200, 30), T[1], bgcolor=(255, 154, 141))
    type3 = pygbutton.PygButton(
        (300, 400, 200, 30), T[2], bgcolor=(255, 154, 141))
...
def TypeList():
    typeList = ['Food', 'Tourism', 'History', 'Geography',
                'Music', 'Wildlife', 'Sports', 'Famous']
...
def dbQueData(screen, questionType):
    ...
    if con.is_connected():
        try:
            cur = con.cursor()
            cur.execute(
                'select min(q_no), max(q_no) from {};'.format(questionType))
            q_range = cur.fetchall()[0]
            q_no = random.randrange(int(q_range[0]), int(q_range[1]))
            cur.execute('select question from {} where q_no = {};'.format(
                questionType, q_no))
            question = cur.fetchone()[0]
            cur.execute(
                'select * from {} where name = {};'.format(questionType+'_options', q_no))
            ...
            if questionType in ('Food', 'Tourism', 'Wildlife'):
                cur.execute('select link from {} where q_no = {};'.format(
                    questionType, q_no))
                link = cur.fetchone()[0]
                if link:
    ...
    while running:
        ...
        for event in pygame.event.get():
            ...
            if answer == Q:
                ...
                msg = FONT.render(
                    "Oops, that is INCORRECT :(", True, (50, 50, 50))
                screen.blit(msg, [370, 520])
                pygame.display.flip()
                pygame.time.delay(1200)
                return False
