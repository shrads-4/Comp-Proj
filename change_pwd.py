import pygame as pg, os, homepage, time, mysql.connector, user_details
    ...
    if con.is_connected():
        try:
            ...
            if new_pwd == confirm_pwd and current_pwd == result[0] and result:
                cur.execute('update user_dets set pwd="{}" where username="{}"'.format(
                    new_pwd, username))
                con.commit()
                return True
            else:
...
def main(username):
    ...
    while not done:
        ...
        screen.blit(FONT.render('Current password',
                                True, (0, 0, 0)), (150, 250))
        screen.blit(FONT.render('New password', True, (0, 0, 0)), (150, 325))
        screen.blit(FONT.render('Confirm password',
                                True, (0, 0, 0)), (150, 400))
        ...
        if 510 > mouse[0] > 290 and 532 > mouse[1] > 500 and click[0] == 1 and not done:
            current_pwd, new_pwd, confirm_pwd = input_boxes[
                0].text, input_boxes[1].text, input_boxes[2].text
